...
void *tree_rate_monitor_func(void *arg) {
    ...
    thread_tree_t *tree = ctx->tree;
    double min_rate = ctx->min_metadata_rate;
    int check_interval = ctx->check_interval_sec > 0 ? ctx->check_interval_sec : 10;
    int grace_period = ctx->grace_period_sec > 0 ? ctx->grace_period_sec : 30;
    int min_lifetime = ctx->min_lifetime_sec;
    bool require_empty = ctx->require_empty_queue;

    log_msg(LOG_DEBUG, "[tree %u] Rate monitor started (min_rate=%.2f/s, min_lifetime=%ds, require_empty=%d)",
            tree->tree_id, min_rate, min_lifetime, require_empty);
    ...
    while (!atomic_load(&tree->shutdown_requested)) {
        ...
        uint64_t count = atomic_load(&tree->metadata_count);

        /* Calculate rate */
        double elapsed = difftime(now, last_time);
        double rate = elapsed > 0 ? (count - last_count) / elapsed : 0;
        tree->metadata_rate = rate;

        last_count = count;
        last_time = now;

        /* Check tree age - must meet minimum lifetime */
        double tree_age = difftime(now, tree->creation_time);
        if (tree_age < min_lifetime) {
            log_msg(LOG_DEBUG, "[tree %u] Metadata rate: %.2f/s (total: %lu, age: %.0fs < min: %ds - IMMUNE)",
                    tree->tree_id, rate, (unsigned long)count, tree_age, min_lifetime);
            continue;  /* Skip rate checks during minimum lifetime */
        }

        /* Get infohash queue status */
        int queue_count = tree_infohash_queue_count(tree->infohash_queue);
        bool queue_empty = (queue_count == 0);

        log_msg(LOG_DEBUG, "[tree %u] Metadata rate: %.2f/s (total: %lu, queue: %d)",
                tree->tree_id, rate, (unsigned long)count, queue_count);

        /* Check if below threshold */
        if (rate < min_rate && count > 0) {  /* Only check after at least one fetch */
            /* Check empty queue requirement */
            if (require_empty && !queue_empty) {
                log_msg(LOG_DEBUG, "[tree %u] Rate %.2f/s below threshold but queue NOT empty (%d items) - SKIPPING shutdown check",
                        tree->tree_id, rate, queue_count);
                continue;  /* Don't shutdown if queue has items */
            }

            log_msg(LOG_WARN, "[tree %u] Metadata rate %.2f/s below threshold %.2f/s%s, entering grace period",
                    tree->tree_id, rate, min_rate, queue_empty ? " AND queue empty" : "");
            ...
            /* Re-check conditions */
            uint64_t count2 = atomic_load(&tree->metadata_count);
            double rate2 = (count2 - count) / (double)grace_period;
            int queue_count2 = tree_infohash_queue_count(tree->infohash_queue);
            bool queue_empty2 = (queue_count2 == 0);

            /* Both conditions must be met for shutdown */
            bool rate_condition = (rate2 < min_rate);
            bool queue_condition = (!require_empty || queue_empty2);

            if (rate_condition && queue_condition) {
                log_msg(LOG_DEBUG, "[tree %u] Shutdown conditions MET: rate=%.2f/s < %.2f/s, queue=%d (empty=%d), requesting shutdown",
                        tree->tree_id, rate2, min_rate, queue_count2, queue_empty2);
                thread_tree_request_shutdown(tree);
                break;
            } else {
                log_msg(LOG_DEBUG, "[tree %u] Shutdown conditions NOT MET: rate=%.2f/s (met=%d), queue=%d (met=%d) - CONTINUING",
                        tree->tree_id, rate2, rate_condition, queue_count2, queue_condition);
            }
        }
    }
