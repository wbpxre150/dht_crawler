...
void *tree_rate_monitor_func(void *arg) {
    ...
    log_msg(LOG_DEBUG, "[tree %u] Rate monitor started (min_rate=%.2f/s)", tree->tree_id, min_rate);
    ...
    while (!atomic_load(&tree->shutdown_requested)) {
        ...
        log_msg(LOG_DEBUG, "[tree %u] Metadata rate: %.2f/s (total: %lu)",
                tree->tree_id, rate, (unsigned long)count);
