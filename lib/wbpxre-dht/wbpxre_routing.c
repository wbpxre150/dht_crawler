...
/* Helper: Add node pointer to flat array (must be called with update_lock held) */
static void add_node_to_flat_array(wbpxre_routing_table_t *table, wbpxre_routing_node_t *node) {
    ...
    for (int i = 0; i < table->all_nodes_capacity; i++) {
        if (table->all_nodes[i] == NULL) {
            /* Use rcu_assign_pointer for safe publication to readers */
            rcu_assign_pointer(table->all_nodes[i], node);
...
int wbpxre_routing_table_insert(wbpxre_routing_table_t *table,
    ...
    bool inserted = false;
    wbpxre_routing_node_t *new_root = insert_node_recursive(table->root, node, &inserted);
...
static wbpxre_routing_node_t *find_node_recursive(wbpxre_routing_node_t *root,
    ...
    if (cmp == 0) {
        ...
        /* Use rcu_dereference for safe pointer load */
        wbpxre_routing_node_t *left = rcu_dereference(root->left);
        return find_node_recursive(left, node_id);
    } else {
        /* Use rcu_dereference for safe pointer load */
        wbpxre_routing_node_t *right = rcu_dereference(root->right);
        return find_node_recursive(right, node_id);
    }
}
...
wbpxre_routing_node_t *wbpxre_routing_table_find(wbpxre_routing_table_t *table,
    ...
    /* Copy node data while holding RCU lock to prevent use-after-free */
    wbpxre_routing_node_t *node_copy = NULL;
    if (node) {
        node_copy = malloc(sizeof(wbpxre_routing_node_t));
        if (node_copy) {
...
uint32_t wbpxre_routing_table_count(wbpxre_routing_table_t *table) {
    if (!table) return 0;
    return (uint32_t)table->node_count;
}
...
int wbpxre_routing_table_get_closest(wbpxre_routing_table_t *table,
    ...
    wbpxre_routing_node_t *root = rcu_dereference(table->root);
    collect_all_nodes_recursive(root, &all_nodes, &count, &capacity, target);
...
int wbpxre_routing_table_get_sample_candidates(wbpxre_routing_table_t *table,
    ...
    for (int i = 0; i < table->all_nodes_capacity && count < n; i++) {
        ...
        /* Use rcu_dereference for safe pointer load from array */
        wbpxre_routing_node_t *node = rcu_dereference(table->all_nodes[idx]);
...
int wbpxre_routing_table_get_low_quality_nodes(wbpxre_routing_table_t *table,
    ...
    int count = 0;
    wbpxre_routing_node_t *root = rcu_dereference(table->root);
    collect_low_quality_nodes_recursive(root, candidates, &count, n, min_rate, min_queries);
...
int wbpxre_routing_table_get_oldest_nodes(wbpxre_routing_table_t *table,
    ...
    int count = 0;
    wbpxre_routing_node_t *root = rcu_dereference(table->root);
    collect_nodes_for_aging_recursive(root, all_nodes, &count, max_collect);
...
int wbpxre_routing_table_get_distant_nodes(wbpxre_routing_table_t *table,
    ...
    int count = 0;
    wbpxre_routing_node_t *root = rcu_dereference(table->root);
    collect_nodes_for_aging_recursive(root, temp_nodes, &count, max_collect);
...
void wbpxre_routing_table_get_keyspace_distribution(wbpxre_routing_table_t *table,
    ...
    *distant_nodes = 0;
    ...
    for (int i = 0; i < table->all_nodes_capacity; i++) {
        wbpxre_routing_node_t *node = rcu_dereference(table->all_nodes[i]);
...
int wbpxre_routing_table_get_distant_nodes_fast(wbpxre_routing_table_t *table,
    ...
    int distant_count = 0;
    wbpxre_routing_node_t *root = rcu_dereference(table->root);
    collect_distant_nodes_recursive(root, current_node_id,
...
dual_routing_controller_t *dual_routing_controller_create(int max_nodes_per_table,
    ...
    controller->table_a_state = DUAL_TABLE_STATE_ACTIVE;
    controller->table_b_state = DUAL_TABLE_STATE_READY;
...
static void perform_rotation(dual_routing_controller_t *controller) {
    ...
    if (new_active == controller->table_a) {
        controller->table_a_state = DUAL_TABLE_STATE_ACTIVE;
        controller->table_b_state = DUAL_TABLE_STATE_CLEARING;
    } else {
        controller->table_b_state = DUAL_TABLE_STATE_ACTIVE;
        controller->table_a_state = DUAL_TABLE_STATE_CLEARING;
    }
    ...
    if (old_active == controller->table_a) {
        controller->table_a_state = DUAL_TABLE_STATE_READY;
    } else {
        controller->table_b_state = DUAL_TABLE_STATE_READY;
    }
...
int dual_routing_insert(dual_routing_controller_t *controller,
    ...
    if (fill_ratio >= controller->fill_start_threshold &&
        ...
        dual_table_state_t other_state = (other_table == controller->table_a)
            ? controller->table_a_state : controller->table_b_state;

        if (other_state == DUAL_TABLE_STATE_READY) {
            controller->filling_table = other_table;

            if (other_table == controller->table_a) {
                controller->table_a_state = DUAL_TABLE_STATE_FILLING;
            } else {
                controller->table_b_state = DUAL_TABLE_STATE_FILLING;
            }
...
void dual_routing_force_rotation(dual_routing_controller_t *controller) {
    ...
    if (other_state == DUAL_TABLE_STATE_READY || other_state == DUAL_TABLE_STATE_FILLING) {
...
int triple_routing_insert_node(
    ...
    if (node_count >= ctrl->rotation_threshold) {
        /* Trigger rotation (internally checks threshold again under lock) */
...
static void triple_routing_try_rotate(triple_routing_controller_t *ctrl) {
    ...
    int next_filling_idx = (filling_idx + 1) % 3;
    while (ctrl->states[next_filling_idx] != TABLE_STATE_IDLE) {
        next_filling_idx = (next_filling_idx + 1) % 3;
        /* Guaranteed to find one: we just set old stable to IDLE */
    }

    /* Clear the IDLE table if it has stale nodes */
    wbpxre_routing_table_t *next_filling_table = ctrl->tables[next_filling_idx];
    uint32_t stale_nodes = wbpxre_routing_table_count(next_filling_table);
    if (stale_nodes > 0) {
        log_msg(LOG_INFO, "  Table %d: IDLE → CLEARING (%u stale nodes)",
                next_filling_idx, stale_nodes);

        ctrl->states[next_filling_idx] = TABLE_STATE_CLEARING;

        /* Release lock during clear (blocking operation) */
        pthread_mutex_unlock(&ctrl->rotation_lock);

        /* Clear table (this is a synchronous blocking operation) */
        clear_routing_table_internal(next_filling_table);
        ctrl->total_nodes_cleared += stale_nodes;
        ctrl->clearing_operations++;

        log_msg(LOG_INFO, "  Table %d: CLEARING → FILLING (cleared %u nodes)",
                next_filling_idx, stale_nodes);
    ...
    log_msg(LOG_INFO, "  Table %d: FILLING (ready for inserts)", next_filling_idx);
...
void triple_routing_get_stats(
    ...
    stats->clearing_operations = ctrl->clearing_operations;
