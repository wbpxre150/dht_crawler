...
static void *maintenance_thread_func(void *arg) {
    ...
    while (dht->running) {
        sleep(5);  /* Run maintenance every 5 seconds */
        ...
        /* ====================================================================
         * Phase 3: Batch Cleanup of Dropped Nodes (configurable interval)
         * ==================================================================== */
        int cleanup_interval = dht->config.node_cleanup_interval_sec > 0 ?
                              dht->config.node_cleanup_interval_sec : 30;
        int cleanup_cycles = (cleanup_interval + 4) / 5;  /* Convert seconds to 5-second cycles */
        if (cycle_count % cleanup_cycles == 0) {
            int removed = wbpxre_routing_table_cleanup_dropped(dht->routing_table);
            (void)removed; /* Suppress unused variable warning */
        }

        /* ====================================================================
         * Phase 2: Active Node Verification
         * Verify old nodes and mark unresponsive ones as dropped
         * ==================================================================== */
        time_t now = time(NULL);
        int max_age = dht->config.max_node_age_sec > 0 ? dht->config.max_node_age_sec : 120;
        time_t verification_threshold = now - max_age;

        /* Get nodes for verification (batch size from config) */
        int batch_size = dht->config.node_verification_batch_size > 0 ?
                         dht->config.node_verification_batch_size : 100;
        wbpxre_routing_node_t **old_nodes = malloc(sizeof(wbpxre_routing_node_t *) * batch_size);
        if (!old_nodes) continue;
        int old_count = wbpxre_routing_table_get_old_nodes(dht->routing_table,
        ...
        /* ====================================================================
         * Phase 4: Quality-based Node Eviction
         * Proactively drop nodes with poor response rates
         * ==================================================================== */
        double min_rate = dht->config.min_node_response_rate > 0.0 ?
                          dht->config.min_node_response_rate : 0.20;
        int min_queries = dht->config.node_quality_min_queries > 0 ?
                          dht->config.node_quality_min_queries : 5;

        /* Get low-quality nodes (up to half the batch size) */
        int quality_batch = batch_size / 2;
        if (quality_batch > 0) {
            wbpxre_routing_node_t **low_quality_nodes = malloc(sizeof(wbpxre_routing_node_t *) * quality_batch);
            if (low_quality_nodes) {
                int quality_count = wbpxre_routing_table_get_low_quality_nodes(dht->routing_table,
                                                                                low_quality_nodes,
                                                                                quality_batch,
                                                                                min_rate, min_queries);

                /* Drop low-quality nodes */
                for (int i = 0; i < quality_count; i++) {
                    wbpxre_routing_table_drop_node(dht->routing_table, low_quality_nodes[i]->id);

                    pthread_mutex_lock(&dht->stats_mutex);
                    dht->stats.nodes_dropped++;
                    pthread_mutex_unlock(&dht->stats_mutex);

                    free(low_quality_nodes[i]);
                }

                free(low_quality_nodes);
            }
        }
        ...
        if (fill_ratio > 0.90) {
            ...
            if (nodes_to_evict > 0) {
                /* Get current node_id for keyspace-aware eviction */
                uint8_t current_node_id[WBPXRE_NODE_ID_LEN];
                pthread_rwlock_rdlock(&dht->node_id_lock);
                memcpy(current_node_id, dht->config.node_id, WBPXRE_NODE_ID_LEN);
                pthread_rwlock_unlock(&dht->node_id_lock);

                wbpxre_routing_node_t **distant_nodes = malloc(sizeof(wbpxre_routing_node_t *) * nodes_to_evict);
                if (distant_nodes) {
                    /* Use keyspace-aware eviction: prioritize distant nodes */
                    int distant_count = wbpxre_routing_table_get_distant_nodes(dht->routing_table,
                                                                                 current_node_id,
                                                                                 distant_nodes,
                                                                                 nodes_to_evict);
                    ...
                    for (int i = 0; i < distant_count; i++) {
                        wbpxre_routing_table_drop_node(dht->routing_table, distant_nodes[i]->id);
                    ...
                    /* Log capacity-based eviction for debugging */
                    if (distant_count > 0 && cycle_count % 6 == 0) {  /* Log every 30 seconds */
                        printf("[wbpxre-dht] Capacity eviction: dropped %d/%d distant nodes (%.1f%% full -> target 70%%)\n",
                               distant_count, nodes_to_evict, fill_ratio * 100.0);
                    }
        ...
        if (dht->config.bep51_pruning_enabled &&
            ...
            if (fill_ratio > min_capacity) {
                ...
                /* CRITICAL PATH: >95% full - force prune oldest 20% immediately */
                if (fill_ratio > 0.95) {
                    nodes_to_remove = (int)(max_nodes * 0.20);

                    printf("[wbpxre-dht] CRITICAL: Routing table at %.1f%% capacity (%d/%d nodes)\n",
                           fill_ratio * 100.0, current_nodes, max_nodes);
                    printf("[wbpxre-dht] CRITICAL: Force-pruning oldest %d nodes (20%% of capacity)\n",
                           nodes_to_remove);

                    /* Get oldest nodes and prune ALL of them (no filtering) */
                    wbpxre_routing_node_t **oldest_nodes =
                    ...
                    if (oldest_nodes) {
                        ...
                        printf("[wbpxre-dht] CRITICAL: Retrieved %d oldest nodes for removal\n", old_count);

                        /* Drop ALL oldest nodes - no threshold checks */
                        for (int i = 0; i < old_count; i++) {
                        ...
                        printf("[wbpxre-dht] CRITICAL: Removed %d oldest nodes\n", total_pruned);
                    }

                }
                /* AGGRESSIVE PATH: 90-95% full - two-phase pruning */
                    ...
                    if (remaining_to_prune > 0) {
                        ...
                        if (old_nodes) {
                            ...
                            for (int i = 0; i < old_count && total_pruned < nodes_to_remove; i++) {
                                ...
                                /* Only prune if unresponsive >120s */
                ...
                /* MODERATE PATH: Below 90% but above min_capacity */
                else {
                ...
                if (total_pruned > 0) {
                    ...
                    printf("[wbpxre-dht] Pruning complete: "
        ...
        /* Get some nodes from routing table (returns copies to avoid data races) */
        wbpxre_routing_node_t *nodes[8];
        int count = wbpxre_routing_table_get_closest(dht->routing_table,
                                                     random_target, nodes, 8);

        /* Send find_node to a few random nodes */
        for (int i = 0; i < count && i < 3; i++) {
            wbpxre_routing_node_t *dummy_nodes = NULL;
            int dummy_count = 0;
            wbpxre_protocol_find_node(dht, &nodes[i]->addr.addr, random_target,
                                     &dummy_nodes, &dummy_count);

            /* Insert discovered nodes into routing table */
            for (int j = 0; j < dummy_count; j++) {
                /* Make a copy to avoid use-after-free if insert holds reference */
                wbpxre_routing_node_t node_copy = dummy_nodes[j];
                wbpxre_routing_table_insert(dht->routing_table, &node_copy);
            }
        ...
        /* Also search for our own node ID (neighborhood refresh) */
        count = wbpxre_routing_table_get_closest(dht->routing_table,
                                                 local_node_id, nodes, 8);
        if (count > 0) {
            wbpxre_routing_node_t *dummy_nodes = NULL;
            int dummy_count = 0;
            wbpxre_protocol_find_node(dht, &nodes[0]->addr.addr, local_node_id,
                                     &dummy_nodes, &dummy_count);
